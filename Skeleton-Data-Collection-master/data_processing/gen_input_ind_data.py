...
class GenInputData:
    ...
    def save_data(self, data_type='train'):
        if data_type == 'train':
            ...
            input_save_name = os.path.join('/media/zhenyue-qin/Elements/Data-Collection/Skeleton-Data-Collection/data_processing/test_feeding_data', 'trn_individual_data.npy')
            label_save_name = os.path.join('/media/zhenyue-qin/Elements/Data-Collection/Skeleton-Data-Collection/data_processing/test_feeding_data', 'trn_individual_label.pkl')
        elif data_type == 'val':
            tgt_cameras = self.val_cameras
            input_save_name = os.path.join('/media/zhenyue-qin/Elements/Data-Collection/Skeleton-Data-Collection/data_processing/test_feeding_data', 'val_individual_data.npy')
            label_save_name = os.path.join('/media/zhenyue-qin/Elements/Data-Collection/Skeleton-Data-Collection/data_processing/test_feeding_data', 'val_individual_label.pkl')
        ...
        all_np_tensor = open_memmap(
            ...
            dtype='float32',
            mode='w+',
            shape=(2*len(npy_file_list), 3, 300, 32, 1))
        ...
        for an_idx, a_npy_f_p in enumerate(npy_file_list):
            ...
            if file_name not in asymmetric_action_codes:
                an_label = self.action_label_dict[file_name]
            ...
            skeleton_0 = self.pre_normalization(np.expand_dims(np.expand_dims(skeleton_0, axis=0), axis=-1))
            skeleton_1 = self.pre_normalization(np.expand_dims(np.expand_dims(skeleton_1, axis=0), axis=-1))

            all_np_tensor[an_idx*2, :C, :T, :V, :1] = skeleton_0
            all_np_tensor[an_idx*2+1, :C, :T, :V, :1] = skeleton_1

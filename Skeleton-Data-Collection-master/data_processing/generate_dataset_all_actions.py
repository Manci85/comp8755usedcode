...
from action_info import action_independent_list
...
class GenInputData:
    def __init__(self):
        self.exp_dirs = \
            list(get_immediate_subdirectories(
                '/media/manci/projectdata/output_blur/2021-05-17T14h00-16h00'
            ))
    ...
    def gen_action_label_dict(self):
        ...
        for a_i in range(1, 2+1):
            for g_i in range(1, 40+1):
                ...
                if action_code in action_independent_list:
                    self.action_label_dict[action_code + '_l'] = count_
                    count_ += 1
                    self.action_label_dict[action_code + '_r'] = count_
                    count_ += 1
                else:
    ...
    def save_data(self, data_type='train'):
        if data_type == 'train':
            ...
            input_save_name = os.path.join('/media/manci/projectdata/test',
                                           'trn_ind_data_non_violent.npy')
            label_save_name = os.path.join('/media/manci/projectdata/test',
                                           'trn_ind_label_non_violent.pkl')
        elif data_type == 'val':
            tgt_cameras = self.val_cameras
            input_save_name = os.path.join('/media/manci/projectdata/test',
                                           'val_ind_data_non_violent.npy')
            label_save_name = os.path.join('/media/manci/projectdata/test',
                                           'val_ind_label_non_violent.pkl')
        else:
        ...
        for an_idx, a_npy_f_p in enumerate(npy_file_list):
            ...
            if file_name in action_independent_list:
                tgt_file_name_num += 2
            else:
                tgt_file_name_num += 1
        ...
        for an_idx, a_npy_f_p in enumerate(npy_file_list):
            ...
            # 如果这个action pair是两个independent动作
            if file_name in action_independent_list:
                # 如果这个pair中两个action是独立的, 就只用第一个人的位置来保存.
                skeleton_0_zero = np.zeros((1, 3, 300, 32, 2))
                skeleton_0_zero[:, :, :skeleton_0.shape[2], :, 0:1] = skeleton_0
                skeleton_1_zero = np.zeros((1, 3, 300, 32, 2))
                skeleton_1_zero[:, :, :skeleton_1.shape[2], :, 0:1] = skeleton_1
                skeleton_0_zero = self.pre_normalization(skeleton_0_zero)
                skeleton_1_zero = self.pre_normalization(skeleton_1_zero)
                # 我们需要对 all_np_tensor 插入两次skeleton, 因为有两个action.
                all_np_tensor[idx_count] = skeleton_0_zero
                all_np_tensor[idx_count + 1] = skeleton_1_zero

                # 标签要插入两次, 因为有两个action.
                label_l = self.action_label_dict[file_name + '_l']
                label_r = self.action_label_dict[file_name + '_r']

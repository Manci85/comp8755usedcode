...
class SkeletonDataGui:
    def __init__(self):
        ...
        self.action_entrybox = ttk.Entry(self.win, width=50, textvariable=self.action_var)
        ...
        self.backward_button = ttk.Radiobutton(self.win, text='Backward', value='Backward',
        ...
        self.record4 = "---------- record will be shown here ----------"
        self.record5 = "---------- record will be shown here ----------"

        self.list_box_label = ttk.Label(self.win, text="The latest five records... ")
        ...
        self.listbox.insert(3, self.record4)
        self.listbox.insert(4, self.record5)
    ...
    def update_listbox(self, a_start_time, a_end_time, a_last_time, a_orientation_var, a_active_var):
        ...
        self.record3 = self.record4
        self.record4 = self.record5
        self.record5 = cur_record

        self.listbox.delete(0, tk.END)
        self.listbox.insert(0, self.record1)
        if "[Deleted record]" in self.record1:
            self.listbox.itemconfig(0, {'bg': 'red'})

        self.listbox.insert(1, self.record2)
        if "[Deleted record]" in self.record2:
            self.listbox.itemconfig(1, {'bg': 'red'})

        self.listbox.insert(2, self.record3)
        if "[Deleted record]" in self.record3:
            self.listbox.itemconfig(2, {'bg': 'red'})

        self.listbox.insert(3, self.record4)
        if "[Deleted record]" in self.record4:
            self.listbox.itemconfig(3, {'bg': 'red'})

        self.listbox.insert(4, self.record5)
        if "[Deleted record]" in self.record5:
            self.listbox.itemconfig(4, {'bg': 'red'})
    ...
    def del_selected_item(self):
        ...
        if index == 2:
            ...
            self.listbox.itemconfig(2, {'bg': 'red'})
        if index == 3:
            self.record4 = '[Deleted record] ' + self.record4
            self.listbox.insert(3, self.record4)
            self.listbox.itemconfig(3, {'bg': 'red'})
        if index == 4:
            self.record5 = '[Deleted record] ' + self.record5
            self.listbox.insert(4, self.record5)
            self.listbox.itemconfig(4, {'bg': 'red'})
        return None

    def delete_item_bg_setter(self):
        for i in range(3):
            selected_text = self.listbox.get(i)
            if "[Deleted record]" in selected_text:
    ...
    def start_action(self):
        ...
        if not orientation_var_:
            messagebox.showerror(title='Error',
                                 message='Error. Orientation radio empty. Please select Forward/Backward')
            return
        if not active_var_:

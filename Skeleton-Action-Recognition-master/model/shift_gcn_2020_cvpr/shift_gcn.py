...
import sys
sys.path.append("./model/shift_gcn/Temporal_shift/")

from model.shift_gcn_2020_cvpr.Temporal_shift.cuda.shift import Shift
...
class Shift_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3):
        ...
        self.Feature_Mask = nn.Parameter(torch.ones(1,sklt_jnt_num,in_channels, requires_grad=True,device='cuda'),requires_grad=True)
        nn.init.constant(self.Feature_Mask, 0)

        self.bn = nn.BatchNorm1d(sklt_jnt_num*out_channels)
        self.relu = nn.ReLU()
        ...
        index_array = np.empty(sklt_jnt_num*in_channels).astype(np.int)
        for i in range(sklt_jnt_num):
            for j in range(in_channels):
                index_array[i*in_channels + j] = (i*in_channels + j + j*in_channels)%(in_channels*sklt_jnt_num)
        self.shift_in = nn.Parameter(torch.from_numpy(index_array),requires_grad=False)

        index_array = np.empty(sklt_jnt_num*out_channels).astype(np.int)
        for i in range(sklt_jnt_num):
            for j in range(out_channels):
                index_array[i*out_channels + j] = (i*out_channels + j - j*out_channels)%(out_channels*sklt_jnt_num)

...
class Model(nn.Module):
    ...
    def forward(self, x, set_to_fc_last=True):
        ...
        if self.ablation == 'original':
            ...
            ###### First Component ######
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_0-tconv_signal.pt')

            x = self.sgcn1_msgcn(x)
            x = self.sgcn1_ms_tcn_1(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_1-tconv_signal.pt')
            x = self.sgcn1_ms_tcn_2(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_2-tconv_signal.pt')
            x = self.nonlinear_f(x)
            x = self.tcn1(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_3-tconv_signal.pt')
            ###### End First Component ######

            ###### Second Component ######
            x = self.sgcn2_msgcn(x)
            x = self.sgcn2_ms_tcn_1(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_4-tconv_signal.pt')
            x = self.sgcn2_ms_tcn_2(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_5-tconv_signal.pt')
            x = self.nonlinear_f(x)
            x = self.tcn2(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_6-tconv_signal.pt')
            ###### End Second Component ######
            ...
            x = self.sgcn3_msgcn(x)
            x = self.sgcn3_ms_tcn_1(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_7-tconv_signal.pt')
            x = self.sgcn3_ms_tcn_2(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_8-tconv_signal.pt')
            x = self.nonlinear_f(x)
            x = self.tcn3(x)
            torch.save(x, 'eval/tensor_analysis/sklt_0-layer_9-tconv_signal.pt')
            ###### End Third Component ######

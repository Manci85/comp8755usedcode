...
from utils_dir.utils_visual import azure_kinect_post_visualize, plot_multiple_lines
...
class Processor():
    ...
    def train(self, epoch, save_model=False, save_training_score=False):
        ...
        for batch_idx, (data, label, index) in enumerate(loader):
            ...
            for i in range(splits):
                ...
                # Frame idx GT
                # frame_idx_gt = torch.range(1, output.shape[-1]).unsqueeze(0).\
                #     repeat(output.shape[0], 1).to(output.device) / 10.0

                # Loss
                # loss = self.loss(output, frame_idx_gt) / splits
                loss = torch.tensor(0.0).to(data.device)
                for i in range(output.shape[-1]-1):
                    loss += torch.sum(torch.relu(output[:, i] - output[:, i + 1] + diff_threshold))
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                for batch_idx, (data, label, index) in enumerate(self.data_loader[ln]):
                    ...
                    if isinstance(output, tuple):
                        ...
                        l1 = 0
                    frame_idx_gt = torch.arange(1, output.shape[-1]+1).unsqueeze(0). \
                        repeat(output.shape[0], 1).to(output.device) / 10.0

                    # loss = self.loss(output, frame_idx_gt)
                    loss = torch.tensor(0.0).to(data.device)
                    for i in range(output.shape[-1] - 1):
                        loss += torch.sum(torch.relu(output[:, i] - output[:, i + 1] + diff_threshold))

                    # for instance_idx in range(len(output)):
                    #     out_line = output[instance_idx].cpu().numpy()
                    #     # gt_line = frame_idx_gt[instance_idx].cpu().numpy()
                    #     plot_multiple_lines([out_line],
                    #                         labels=['Value'],
                    #                         save_name='test_fields/frame_chronological_order/'
                    #                                   'epoch_{}_{}.png'.format(epoch, instance_idx))
            ...
            print('loss example: ', torch.mean(output, 0))
            out_line = output[0].cpu().numpy()
            plot_multiple_lines([out_line], labels=['Rand Enc'],
                                save_name='test_fields/frame_chronological_order/'
                                          'rand_enc_epoch_{}.png'.format(epoch))

...
class Processor():
    ...
    def __init__(self, arg):
        ...
        arg.model_saved_name = os.path.join(arg.work_dir, 'save_models', arg.Experiment_name)
        if not os.path.exists(arg.model_saved_name):
    ...
    def load_model(self):
        ...
        self.model = Model(**self.arg.model_args,
                           is_use_rank_pool=self.is_use_rank_pool
                           ).cuda(output_device)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            output, other_outs = self.model(data, keep_prob)

            loss_cls_ce = self.loss(output, label)
            loss = loss_cls_ce
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    output, other_outs = self.model(data)
            ...
            if accuracy > self.best_acc:
                ...
                eval_path = self.arg.work_dir + arg.Experiment_name + '/eval_results/'
                if not os.path.exists(eval_path):
                    os.makedirs(eval_path)
                with open(self.arg.work_dir + arg.Experiment_name + '/eval_results/best_acc' + '.pkl'.format(

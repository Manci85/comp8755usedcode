    ...
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        ...
        self.pts_linears = nn.ModuleList(
            [DenseLayer(input_ch, W, activation="relu")] + [
                DenseLayer(W, W, activation="relu") if i not in self.skips else DenseLayer(W + input_ch, W,
                                                                                           activation="relu") for i in
                range(D - 1)])
...
if __name__ == '__main__':
    ...
    an_embedder = Embedder(**embed_kwargs)
    inpt_tmp = torch.randn(5, 3, 100, 25, 2)
    tmp_ = an_embedder.embed(inpt_tmp, dim=2)
    print('tmp_: ', tmp_.shape)

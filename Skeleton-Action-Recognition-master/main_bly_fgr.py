...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/nturgbd-cross1-view/test_bone.yaml',
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if arg.phase == 'train':
            ...
            if not arg.train_feeder_args['debug']:
                ...
                self.val_writer = SummaryWriter(os.path.join(logdir, 'val'), 'val')
        elif arg.phase == 'get_model_features':
    ...
    def load_model(self):
        ...
        #                                          (int(self.arg.model_args['in_channels']), 300, 25, 2), as_strings=True,
    ...
    def train(self, epoch, save_model=False, save_training_score=False):
        ...
        for batch_idx, (data, label, index) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                act_l, lft_l, _ = label
                act_l = act_l.long().cuda(self.output_device)
                lft_l = lft_l.long().cuda(self.output_device)
            ...
            for i in range(splits):
                ...
                batch_data, act_label, l_label = \
                    data[left:right], act_l[left:right], lft_l[left:right]

                # forward
                act_out, bly_fgr_out = self.model(batch_data)

                # 记录training的out文件
                tr_score_batches.append(act_out.data.cpu().numpy())

                # 就是每一个forward batch的loss变小,
                loss_act = self.loss(act_out, act_label) / splits
                loss_fgr = self.loss(bly_fgr_out, lft_l) / splits
                loss = loss_act + loss_fgr
                ...
                value_act, predict_label_act = torch.max(act_out, 1)
                value_fgr, predict_label_fgr = torch.max(bly_fgr_out, 1)
                acc_act = torch.mean((predict_label_act == act_label).float())
                acc_fgr = torch.mean((predict_label_fgr == lft_l).float())

                self.train_writer.add_scalar('acc_act', acc_act, self.global_step)
                self.train_writer.add_scalar('acc_fgr', acc_fgr, self.global_step)
                self.train_writer.add_scalar('loss_acc', loss_act.item() * splits, self.global_step)
                self.train_writer.add_scalar('loss_fgr', loss_act.item() * splits, self.global_step)
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                for batch_idx, (data, label, index) in enumerate(self.data_loader[ln]):
                    ...
                    act_out, bly_fgr_out = self.model(data)

                    # # 看一看为什么是FGR是0
                    # _, predict_label_fgr = torch.max(bly_fgr_out.data, 1)
                    # fgr_correct_ = torch.sum(predict_label_fgr == lft_l) / len(lft_l)
                    # print('fgr correct_: ', fgr_correct_)

                    loss_act = self.loss(act_out, act_l).item()
                    loss_fgr = self.loss(bly_fgr_out, lft_l).item()
                    loss = loss_act + loss_fgr

                    # 记录一下在这整个epoch里面输出的结果
                    score_act_batches.append(act_out.data.cpu().numpy())
                    score_fgr_batches.append(bly_fgr_out.data.cpu().numpy())

                    # 记录一下loss
                    loss_acc_values.append(loss_act)
                    loss_fgr_values.append(loss_fgr)
                    loss_values.append(loss)

                    step += 1

            # 把整个epoch的输出给连起来
            score_act = np.concatenate(score_act_batches)
            score_fgr = np.concatenate(score_fgr_batches)

            # 得到accuracy
            accuracy_act, accuracy_fgr = self.data_loader[ln].dataset.top_k((score_act, score_fgr), 1)

            loss_act = np.mean(loss_act)
            loss_fgr = np.mean(loss_fgr)
            loss = np.mean(loss_values)
            print('loss_act: ', loss_act)
            print('loss_fgr: ', loss_fgr)

            print('accuracy act: ', accuracy_act)
            print('best acc: ', self.best_acc)
            if accuracy_act >= self.best_acc:
                self.best_acc = accuracy_act
                self.best_top_5_acc, _ = self.data_loader[ln].dataset.top_k((score_act, score_fgr), 5)
                self.best_acc_epoch = epoch + 1
                save_score = True

            print('Accuracy Action: ', accuracy_act,
                  'Accuracy Fine Grain: ', accuracy_fgr,
                  ' model: ', self.arg.work_dir)
            if self.arg.phase == 'train' and not self.arg.debug:
                self.val_writer.add_scalar('acc_act', accuracy_act, self.global_step)
                self.val_writer.add_scalar('acc_fgr', accuracy_fgr, self.global_step)
                self.val_writer.add_scalar('loss_acc', loss_act.item(), self.global_step)
                self.val_writer.add_scalar('loss_fgr', loss_act.item(), self.global_step)
                self.val_writer.add_scalar('loss', loss.item() , self.global_step)

            score_dict = dict(zip(self.data_loader[ln].dataset.sample_name, score_act))
            self.print_log(f'\tMean {ln} loss of {len(self.data_loader[ln])} batches: {np.mean(loss_values)}.')
            for k in self.arg.show_topk:
                acc_top_k, fgr_top_k = self.data_loader[ln].dataset.top_k((score_act, score_fgr), k)
                self.print_log(f'\tTop {k} ACT: {100 * acc_top_k:.2f}%')
                self.print_log(f'\tTop {k} FGR: {100 * fgr_top_k:.2f}%')
            self.print_log(f'\tCurrent Best top-1 accuracy: {100 * self.best_acc:.2f}%')

            # get confusion matrix
            if self.best_acc_epoch == epoch + 1:
                if 'bly-fgr' in self.arg.train_feeder_args['data_path']:
                    acc_dir = os.path.join(self.arg.work_dir, 'accuracy_info')
                    if not os.path.exists(acc_dir):
                        os.makedirs(acc_dir)
                    acc_f_name_prefix = '{}/epoch_{}_{}'.format(acc_dir, epoch + 1, ln)
                    predicted_labels = score_act.argsort()[:, -1]
